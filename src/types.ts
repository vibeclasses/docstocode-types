...
export const FEATURE_STATUSES = ['backlog', 'planning', 'in-progress', 'testing', 'completed'] as const;
export const BUG_STATUSES = ['open', 'in-progress', 'resolved', 'closed', 'wont-fix'] as const;
export const TASK_STATUSES = ['todo', 'in-progress', 'blocked', 'completed'] as const;
...
export const isFeature = (item: ProjectItem): item is Feature => item.type === 'feature';
...
export type CreateItemInput<T extends ProjectItem> = Omit<T, 'id' | 'createdAt' | 'updatedAt'> & {
  id?: string;
};

// Utility type for updating items
export type UpdateItemInput<T extends ProjectItem> = Partial<Omit<T, 'id' | 'type' | 'createdAt'>> & {
  id: string;
};

// Status transition maps using const assertions
export const FEATURE_STATUS_TRANSITIONS: Record<FeatureStatus, readonly FeatureStatus[]> = {
  backlog: ['planning'],
